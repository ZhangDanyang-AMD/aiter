...
def mxfp8_attention_forward(
    ...
    o_shape[-1] = v.shape[-1]
    o = torch.empty(o_shape, device=q.device, dtype=torch.bfloat16 if use_mxfp8 else q.dtype, requires_grad=True)
    ...
    if use_mxfp8:
        ...
        stride_qdescale_z = stride_qdescale_h = stride_qdescale_m = stride_qdescale_d = None
        stride_kdescale_z = stride_kdescale_h = stride_kdescale_m = stride_kdescale_d = None
        stride_vdescale_z = stride_vdescale_h = stride_vdescale_m = stride_vdescale_d = None
    ...
    attn_fwd_mxfp8[grid](
        q, k, v, bias, p_scale, q_scale, k_scale, v_scale, use_mxfp8, sm_scale, softmax_lse, o,
        *q_strides, *k_strides, *v_strides, *o_strides, *bias_strides, *alibi_strides, *scores_strides,
        ...
        dropout_p=dropout_p, philox_seed=philox_seed, philox_offset_base=philox_offset,
        scores=scores, scores_scaled_shifted=scores_scaled_shifted, exp_scores=exp_scores,
        ...
        USE_BIAS=bias is not None, USE_ALIBI=alibi_slopes is not None,
        ENABLE_DROPOUT=dropout_p > 0.0, USE_EXP2=use_exp2, RETURN_SCORES=return_softmax,
...
def mxfp8_attention_backward(
    ...
    if use_mxfp8:
        ...
        do_fp8 = None
        do_scale = None
        stride_dodescalez = stride_dodescaleh = stride_dodescalem = stride_dodescaled = None
    ...
    _bwd_preprocess_use_o_mxfp8[grid_prebwd](
        ...
        BLOCK_M=preprocess_o_block, BLOCK_DMODEL_V=padded_d_model_v,
        ACTUAL_BLOCK_DMODEL_V=head_size_v, N_CTX_Q=max_seqlen_q,
        Z=batch, HQ=nheads_q, IS_VARLEN=is_varlen,
        F8_BWD_DTYPE=tl_f8_bwd_dtype, QUANT_BLOCK_SIZE=quant_block_size,
    )
    ...
    _bwd_kernel_dq_mxfp8[grid_bwd](
        ...
        batch, nheads_q, nheads_k, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k,
        num_block_m=num_block_m,
        BLOCK_M=block_m_dq_bwd, BLOCK_N=block_n_dq_bwd,
        BLOCK_DMODEL_QK=padded_d_model_qk, BLOCK_DMODEL_V=padded_d_model_v,
        ACTUAL_BLOCK_DMODEL_QK=head_size_qk, ACTUAL_BLOCK_DMODEL_V=head_size_v,
        CAUSAL=causal, USE_EXP2=use_exp2, IS_VARLEN=is_varlen,
        use_mxfp8=use_mxfp8, F8_BWD_DTYPE=tl_f8_bwd_dtype,
        QUANT_BLOCK_SIZE=quant_block_size, QUANT_SIZE=quant_size,
        **kernel_kwargs,
    )
