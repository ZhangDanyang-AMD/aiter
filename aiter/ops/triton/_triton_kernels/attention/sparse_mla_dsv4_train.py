...
def _sparse_mla_fwd_kernel(
    q_ptr, kv_ptr, indices_ptr, attn_sink_ptr, out_ptr, lse_ptr,
    q_stride_t: tl.int64, q_stride_h: tl.int64,
    kv_stride_n: tl.int64,
    out_stride_t: tl.int64, out_stride_h: tl.int64,
    lse_stride_t: tl.int64,
    idx_stride_t: tl.int64,
    num_heads: tl.int32, head_dim: tl.int32, num_kv: tl.int32, topk: tl.int32,
    scale,
    ...
    q = tl.load(
        q_ptr + query_idx * q_stride_t
        + head_offsets[:, None] * q_stride_h + dim_offsets[None, :],
    ...
    for k_start in tl.range(0, topk, BLOCK_K):
        ...
        in_range = k_pos < topk
        slot = tl.load(indices_ptr + query_idx * idx_stride_t + k_pos,
        ...
        p = tl.where(m_new[:, None] == float("-inf"), 0.0,
    ...
    if HAS_ATTN_SINK:
        sink = tl.load(attn_sink_ptr + head_offsets, mask=head_mask,
        ...
        out = tl.where(l_final[:, None] > 0.0,
                        (acc * alpha[:, None]) / denom[:, None], 0.0)
    ...
    tl.store(
        out_ptr + query_idx * out_stride_t
        + head_offsets[:, None] * out_stride_h + dim_offsets[None, :],
        out, mask=head_mask[:, None] & dim_mask[None, :],
    )
    tl.store(lse_ptr + query_idx * lse_stride_t + head_offsets,
             lse, mask=head_mask)
...
def _bwd_dq_store_dp_kernel(
    q_ptr, kv_ptr, do_ptr, indices_ptr, lse_ptr, delta_ptr,
    attn_sink_ptr, dq_ptr, d_sink_ptr, p_buf_ptr, dp_buf_ptr,
    q_stride_t: tl.int64, q_stride_h: tl.int64,
    kv_stride_n: tl.int64,
    do_stride_t: tl.int64, do_stride_h: tl.int64,
    idx_stride_t: tl.int64,
    lse_stride_t: tl.int64,
    delta_stride_t: tl.int64,
    dq_stride_t: tl.int64, dq_stride_h: tl.int64,
    p_stride_t: tl.int64, p_stride_h: tl.int64,
    num_heads: tl.int32, head_dim: tl.int32, num_kv: tl.int32, topk: tl.int32,
    scale,
    HAS_ATTN_SINK: tl.constexpr,
    ...
    q = tl.load(
        q_ptr + tok_idx * q_stride_t + h_offsets[:, None] * q_stride_h + d_offsets[None, :],
        mask=h_mask[:, None] & d_mask[None, :], other=0.0,
    )
    do = tl.load(
        do_ptr + tok_idx * do_stride_t + h_offsets[:, None] * do_stride_h + d_offsets[None, :],
        mask=h_mask[:, None] & d_mask[None, :], other=0.0,
    )
    delta = tl.load(delta_ptr + tok_idx * delta_stride_t + h_offsets, mask=h_mask, other=0.0)
    ...
    for k_start in tl.range(0, topk, BLOCK_K):
        k_pos = k_start + k_offsets
        in_range = k_pos < topk
        slot = tl.load(indices_ptr + tok_idx * idx_stride_t + k_pos,
                        mask=in_range, other=-1)
        valid = in_range & (slot >= 0) & (slot < num_kv)
    ...
    tl.store(
        dq_ptr + tok_idx * dq_stride_t + h_offsets[:, None] * dq_stride_h + d_offsets[None, :],
        acc_dq, mask=h_mask[:, None] & d_mask[None, :],
    )
...
def _bwd_dkv_interm_kernel(
    q_ptr, do_ptr, p_buf_ptr, dp_buf_ptr, interm_ptr,
    q_stride_t: tl.int64, q_stride_h: tl.int64,
    do_stride_t: tl.int64, do_stride_h: tl.int64,
    p_stride_t: tl.int64, p_stride_h: tl.int64,
    interm_stride_t: tl.int64, interm_stride_k: tl.int64,
    num_heads: tl.int32, head_dim: tl.int32, topk: tl.int32,
    BLOCK_H: tl.constexpr, BLOCK_D: tl.constexpr, BLOCK_K: tl.constexpr,
    NUM_HG: tl.constexpr,
):
    tok_idx = tl.program_id(0)
    ...
    for k_start in tl.range(0, topk, BLOCK_K):
        ...
        for hg in tl.range(0, NUM_HG):
            ...
            acc_interm += (tl.dot(tl.trans(dp), q)
                           + tl.dot(tl.trans(p), do))

        interm_base = tok_idx * interm_stride_t + k_pos[:, None] * interm_stride_k + d_offsets[None, :]
...
def _bwd_dkv_gather_kernel(
    interm_ptr, inv_ptr_ptr, inv_data_ptr, dkv_ptr,
    interm_stride_t: tl.int64, interm_stride_k: tl.int64,
    dkv_stride_n: tl.int64,
    head_dim: tl.int32, topk: tl.int32, num_tokens: tl.int32,
    BLOCK_D: tl.constexpr, BLOCK_G: tl.constexpr,
):
    ...
    for g_start in tl.range(0, end - start, BLOCK_G):
        ...
        byte_offsets = (flat_pos // topk) * interm_stride_t + (flat_pos % topk) * interm_stride_k
